// Rust port of dlmalloc. You can find the original source at
...
impl<A: DlmallocAllocator> Dlmalloc<A> {
...
impl Chunk {
    ...
    unsafe fn next(me: *mut Chunk) -> *mut Chunk {
        unsafe { (me as *mut u8).add((*me).head & !FLAG_BITS) as *mut Chunk }
    }
    ...
    unsafe fn plus_offset(me: *mut Chunk, offset: usize) -> *mut Chunk {
        unsafe { (me as *mut u8).add(offset) as *mut Chunk }
    }
...
mod tests {
    ...
    unsafe fn setup_treemap<A: DlmallocAllocator>(a: &mut Dlmalloc<A>) {
