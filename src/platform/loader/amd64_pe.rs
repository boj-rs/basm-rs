        ...
        while off < end_of_block {
            ...
            match w_type {
                IMAGE_REL_BASED_HIGH => {
                    ptr::write(patch_addr as *mut u16, ptr::read(patch_addr as *const u16) + (reloc_delta >> 16) as u16);
                },
                IMAGE_REL_BASED_LOW => {
                    ptr::write(patch_addr as *mut u16, ptr::read(patch_addr as *const u16) + reloc_delta as u16);
                },
                IMAGE_REL_BASED_HIGHLOW => {
                    ptr::write(patch_addr as *mut u32, ptr::read(patch_addr as *const u32) + reloc_delta as u32);
                },
                IMAGE_REL_BASED_HIGHADJ => {
                    if off + 2 >= end { unreachable!() }
